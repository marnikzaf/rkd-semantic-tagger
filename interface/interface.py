# --- FIXED & COMPLETE STREAMLIT INTERFACE ---
# This version includes all bug fixes based on your latest feedback.
...
if session_path and "output_ready" in st.session_state:
    ...
    # 'edited_data' is already initialized at the top of the review block
    ...
    selected_title = st.sidebar.selectbox("🔍 Jump to artwork:", options=all_titles, index=st.session_state.index)
    ...
    if "navigate" not in st.session_state:
        st.session_state.navigate = None  # fixed and cleaned
...
st.sidebar.caption(f"🕒 Last saved: {last_saved_time})"").strftime('%H:%M:%S')}")
